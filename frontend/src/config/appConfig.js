...
const LOCAL_API_BASE_URL = 'http://localhost:3000/api'; // Обычно совпадает с основным портом Vite для проксирования
  ...
  if (adminAddress && isAdminPage) {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (isLocalhost) {
          apiUrl = `${LOCAL_API_BASE_URL}/app/config`; // Локальный API сервер
          console.log("[App Config] Приложение запущено локально, используем локальный API сервер для загрузки:", apiUrl);
        } else {
          apiUrl = '/.netlify/functions/getConfig'; // Netlify Functions
          console.log("[App Config] Приложение запущено НЕ локально, используем Netlify Functions для загрузки:", apiUrl);
        }
      } else {
        // Для SSR или Node.js используем Netlify Functions
        apiUrl = '/.netlify/functions/getConfig';
        console.log("[App Config] Приложение запущено в SSR/Node.js, используем Netlify Functions для загрузки:", apiUrl);
      }
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (isLocalhost) {
          apiUrl = `${LOCAL_API_BASE_URL}/app/config`; // Локальный API сервер
          console.log("[App Config] Приложение запущено локально, используем локальный API сервер для загрузки (readonly):", apiUrl);
        } else {
          apiUrl = '/.netlify/functions/getConfigReadOnly'; // Netlify Functions
          console.log("[App Config] Приложение запущено НЕ локально, используем Netlify Functions для загрузки (readonly):", apiUrl);
        }
      } else {
        // Для SSR или Node.js используем Netlify Functions
        apiUrl = '/.netlify/functions/getConfigReadOnly';
        console.log("[App Config] Приложение запущено в SSR/Node.js, используем Netlify Functions для загрузки (readonly):", apiUrl);
      }
  ...
  if (adminAddress) {
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (isLocalhost) {
          apiUrl = `${LOCAL_API_BASE_URL}/app/config`; // Локальный API сервер
          console.log("[App Config] Приложение запущено локально, используем локальный API сервер для сохранения:", apiUrl);
        } else {
          apiUrl = '/.netlify/functions/saveConfig'; // Netlify Functions
          console.log("[App Config] Приложение запущено НЕ локально, используем Netlify Functions для сохранения:", apiUrl);
        }
      } else {
        // Для SSR или Node.js используем Netlify Functions
  ...
  if (typeof window !== 'undefined') {
    ...
    if (isLocalhost) {
      ...
      try {
        ...
        console.log(`[App Config] Ответ от Netlify Function: ${response.status} ${response.statusText}`);
        if (response.ok) {
          const serverResponse = await response.json();
          console.log("[App Config] Полный ответ от Netlify Function (parsed):", JSON.stringify(serverResponse, null, 2));

          // - УТОЧНЕНИЕ ЛОГИКИ -
          // Ранее было: const interval = serverResponse.config ? serverResponse.config.updateIntervalMinutes : serverResponse.updateIntervalMinutes;
          // Теперь предполагаем, что serverResponse - это сама конфигурация
          let interval;
          if (serverResponse && typeof serverResponse === 'object' && serverResponse.updateIntervalMinutes !== undefined) {
             interval = serverResponse.updateIntervalMinutes;
             console.log(`[App Config] Интервал обновления извлечён напрямую из serverResponse.updateIntervalMinutes: ${interval}`);
          } else {
             console.warn("[App Config] updateIntervalMinutes не найден в основном теле ответа serverResponse. ServerResponse:", serverResponse);
             interval = undefined; // Пойдёт к localStorage
          }
          // - КОНЕЦ УТОЧНЕНИЯ -
